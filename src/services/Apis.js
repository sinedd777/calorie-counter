export const addFood = async (foodName, calories, date) => {
    const response = await fetch('http://localhost:5000/foods/add', {
        ...
        body: JSON.stringify(
            {name: foodName, calories: calories, date: date}
        )
    });
...
export const updateFoodItem = async (id, name, calories, date) => {
    const response = await fetch('http://localhost:5000/foods/update', {
        ...
        body: JSON.stringify(
            {id: id, name: name, calories: calories, date: date}
        )
    });
    return response.json();
}
...
export const getWeeklyStats = async (multiplier) => {
    const response = await fetch('http://localhost:5000/foods/stats?multiplier=' + multiplier, {
        method: 'GET',
        headers: {
            'x-access-token': localStorage.getItem('token')
        }
...
export const getWeeklyConsumption = async (multiplier) => {
  const response = await fetch('http://localhost:5000/foods/weekly', {
      method: 'GET',
      headers: {
          'x-access-token': localStorage.getItem('token')
      }
...
export const updateUser = async (username, calorieLimit) => {
    const response = await fetch('http://localhost:5000/users/update', {
        ...
        body: JSON.stringify(
            {username: username, calorieLimit: calorieLimit}
        )
    });
    return response.json();
}
